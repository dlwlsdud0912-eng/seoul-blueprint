...
## 다음 할 일 (새 채팅에서)
1. **크롤링 실행**: `npm run crawl:browser` — 264개 아파트 가격 크롤링 (~20분 소요)
2. **prices.json 검증**: 매매가만 들어있는지 확인 (전세 오염 없는지)
3. **실패 13개 수동 매핑**: 필요시 네이버부동산에서 직접 complexId 찾아 apartments.ts에 추가
4. **빌드 & 배포**: `npm run build` 후 git push → Vercel 자동 배포

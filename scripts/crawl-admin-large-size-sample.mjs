...
const SAMPLE_IDS = [
  'gangnam-e381rf',          // 압구정현대14차
  'seocho-1wqyjxk',          // 래미안원베일리
  'gangnam-cheongdam-zai',   // 청담자이
  'yongsan-oygx09',          // 신동아
  'songpa-1eudn04',          // 잠실주공5단지(재건축)
  'gangnam-daechi-samsung1', // 대치삼성1차
  'gangnam-sangnoksu',       // 상록수아파트
  'gangnam-raemian-blestij', // 래미안블레스티지
  'seongdong-xr5i65',        // 청구강변
  'seocho-s9im68',           // 래미안신반포리오센트
];
...
function readApartments() {
  ...
  return apartments.filter((apartment) => SAMPLE_IDS.includes(apartment.id));
}
...
async function crawlSample(page, apartment) {
...
async function main() {
  ...
  if (apartments.length === 0) {
    throw new Error('No sample apartments found.');
  }
  ...
  try {
    for (let i = 0; i < apartments.length; i += 1) {
      ...
      const prefix = `[${i + 1}/${apartments.length}]`;
      console.log(`${prefix} ${apartment.name} 시작`);

      try {
        const result = await crawlSample(page, apartment);
        ...
        console.warn(`${prefix} ${apartment.name} 실패: ${error.message}`);
      }
  ...
  const outputPath = path.join(__dirname, '..', 'public', 'admin-large-size-sample.json');
  fs.writeFileSync(outputPath, JSON.stringify(payload, null, 2), 'utf-8');
  console.log(`샘플 결과 저장 완료: ${outputPath}`);
}
